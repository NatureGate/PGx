...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input-dir", type=str, metavar='<dir>',
                        help="Directory path to PharmCAT JSONs.",default='')
    ...
    parser.add_argument("-a", "--allele-definition-files", type=str,
                        ...
                        help="(Optional) Path to PharmCAT allele definition JSON files. "
                             "Pattern matching strings should be included.",default='./alleles/')
    ...
    try:
        ...
        m_matcher_jsons: list = glob(str(m_input_dir.joinpath(args.matcher_json_pattern)))
        m_phenotyper_jsons: list = glob(str(m_input_dir.joinpath(args.phenotyper_json_pattern)))
        # get sample list based on json file names
        matcher_json_samples: list = ['.'.join(x.split('.')[:-2]) for x in m_matcher_jsons]
        phenotyper_json_samples: list = ['.'.join(x.split('.')[:-2]) for x in m_phenotyper_jsons]
        ...
        if args.sample_file:
            ...
            json_samples: list = list(set(matcher_json_samples) & set(phenotyper_json_samples) & set(m_samples))
        else:
            print('No sample file found. Reading all samples.')
            json_samples: list = list(set(matcher_json_samples) & set(phenotyper_json_samples))
        ...
        if args.genes is not None:
            m_selected_genes: list = [x for x in args.genes.split(",") if x in m_genes]
        ...
        allele_definition_jsons: list = glob(allele_definition_pattern)
        ...
        if args.concurrent_mode:
            ...
            for one_sample in json_samples:
                ...
                tmp_results: dict = extract_pcat_json(
        ...
        # write to an output
        cols: list = ['Sample', 'Gene', 'Phenotype', 'Activity_Score',
                           'Diplotype',
                           'DPYD_RYR1_Variants', 'DPYD_RYR1_Variant_Functions', 'DPYD_RYR1_Variant_Genotypes',
                           'Haplotype_1', 'Haplotype_2',
                           'Haplotype_1_Functions', 'Haplotype_2_Functions',
