...
def process_data():
    ...
    df_excel['Text'] = df_excel['Text'].apply(process_suggest)
    # 对suggest列进行处理，新增一列
    ...
    # 文件药物名称相同，新列内容相同的数据
    merged_df = pd.merge(merged_for_english, df_excel, left_on=['英文名称','processed_suggest'], right_on=['Related Chemicals','Text'], how='left')
    return merged_df
