...
def get_json_file_names(sample_id: str, matcher_json_list, phenotyper_json_list):
    # get the matcher and phenotyper json names for the sample
    matcher_pattern: str = f"{sample_id}.match.json"
    matcher_match_list: list = [x for x in matcher_json_list if matcher_pattern in x]
    phenotyper_pattern: str = f"{sample_id}.phenotype.json"
    phenotyper_match_list: list = [x for x in phenotyper_json_list if phenotyper_pattern in x]
...
def get_names_and_genotypes(json_entry: dict, reference_genotypes) :

    # set up empty return variables
    haplotype_names: list = []
    alternative_genotypes: list = []
    ...
    if json_entry:
        ...
        tmp_genotypes: set = {y for x in json_entry['sequences'] for y in x.split(';')
...
def extract_pcat_json(matcher_json: str, phenotyper_json: str, genes: list, sample_id: str,
                      reference_genotypes: dict, guideline_source: str = 'CPIC') \
        :
    # read json file
    ...
    for i_gene in genes:
        ...
        if n_diploids == 0:
            ...
            p_toxicity_variants: list = []
            p_toxicity_functions: list = []
            ma_toxicity_variants: list = []
            ma_toxicity_genotypes: list = []
