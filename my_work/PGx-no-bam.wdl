...
workflow PGx{
    input {
        ...
        # File bam_index
        File positions = "/Files/sz_history/longrui/pgx/positions.bed"
        File pharmcat_positions = "/Files/sz_history/longrui/pgx/pharmcat_positions.vcf.gz"
        File pharmcat_positions_index = "/Files/sz_history/longrui/pgx/pharmcat_positions.vcf.gz.csi"
        String SampleID
        ...
        File specific_var = "/Files/sz_history/longrui/pgx/specific_var.vcf.gz"
        File specific_var_index = "/Files/sz_history/longrui/pgx/specific_var.vcf.gz.csi"
    ...
    call merge_phased_vcfs_task {
        input:
            ...
            specific_var_index = specific_var_index,
            phased_vcfs = shapeit4_phase_task.phased_vcf,
            vcf_file = split_vcf.vcf_file,
            extracted_positions_file = extract_positions.extracted_vcf,
            extracted_positions_file_index = extract_positions.extracted_vcf_index,
  ...
  #String filename=basename(file,".vcf")
    output{
        ...
        File outcall_file = merge_phased_vcfs_task.update_outcall
...
task extract_positions{
    ...
    command <<<
        ...
        bcftools view -R ~{positions_bed} ~{vcf_allsite_file} -Oz -o extracted.vcf.gz -W
        bcftools annotate -a ~{pharmcat_positions} -c ID extracted.vcf.gz -Oz -o extracted_positions.vcf.gz -W

    >>>
    output {
        File extracted_vcf = "extracted_positions.vcf.gz"
        File extracted_vcf_index = "extracted_positions.vcf.gz.csi"
    }
...
task split_vcf{
    ...
    command <<<
        ...
        # ref_chr=""
        python /home/stereonote/script/split.py --vcf_file ~{vcf_allsite_file} --vcf_file_index ~{vcf_allsite_file_index}
        
    >>>
    output {
        Array[File] vcf_file = glob("*.vcf.gz")
        Array[File] vcf_file_index = glob("*.vcf.gz.csi")
        Array[File] genetic_map = glob("*.b38.gmap.gz")
        Array[File] reference_panel = glob("/zfsms3/pub/database/ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000G_2504_high_coverage/working/20201028_3202_phased/CCDG_14151_B01_GRM_WGS_2020-08-05_*.filtered.shapeit2-duohmm-phased.vcf.gz")  
        Array[File] reference_panel_index = glob("/zfsms3/pub/database/ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000G_2504_high_coverage/working/20201028_3202_phased/CCDG_14151_B01_GRM_WGS_2020-08-05_*.filtered.shapeit2-duohmm-phased.vcf.gz.tbi")  
    }
    
    runtime {
    #The fixed parameter "docker_url" is used in wdl to specify the image address,Please copy the real url from "Image" and paste it here ,such as"stereonote/stereonote:latest". If you switch the area,please change the image url
    docker_url: "stereonote_hpc/longrui_f113d06526fe43028c52420c9221be51_private:latest"
    #The fixed parameter " req_cpu" is used in wdl to apply for a task running cpu
    req_cpu: 8
    #The fixed parameter " req_memory" is used in wdl to specify the running memory
    req_memory: "1Gi"
...
task merge_phased_vcfs_task {
    input {
        #File phase_dir
        File specific_var
        File specific_var_index
        Array[File] phased_vcfs
        Array[File] vcf_file
        Array[File] unphased_vcfs_index
        File extracted_positions_file
        File extracted_positions_file_index
        File chrM_vcf_gz 
    ...
    command <<<
        ...
        bcftools concat -o merged_phased.vcf  -Oz  ~{sep=' ' phased_vcfs} ~{chrM_vcf}
        bcftools concat -o unannotate_merged_unphased.vcf.gz -Oz  ~{extracted_positions_file} ~{chrM_vcf_gz}
        ...
        bcftools annotate -a ~{extracted_positions_file} -c ID unannotate_merged_phased.vcf.gz -o merged_phased.vcf.gz
        bcftools annotate -a ~{extracted_positions_file} -c ID unannotate_merged_unphased.vcf.gz -o merged_unphased.vcf.gz
        bcftools index merged_phased.vcf.gz
        bcftools index merged_unphased.vcf.gz
        bcftools annotate -a merged_phased.vcf.gz -c FORMAT/GT -Ov -o annotated.vcf merged_unphased.vcf.gz
        bcftools isec ~{chrM_vcf_gz}  ~{specific_var}  -p specific_var
        bcftools view -i "ID == 'rs267606618' || ID == 'rs267606619' || ID == 'rs267606617' || ID == 'rs28358569'" specific_var/0003.vcf|grep -v '^#'|awk -F'\t' '{print $3}'|tail -n1 > MTRNR1.txt
    ...
    output {
        ...
        File specific_var_file = "specific_var/0002.vcf"
...
task pharmCatTask{
  ...
  command <<<
    ...
    source activate base
    awk 'BEGIN{FS=OFS="\t"} $6=="inf"{$6="."} 1' ~{file} > ~{filename}.preprocessed.vcf
    java -Xmx1g -jar /home/stereonote/pharmcat-3.0.1-all.jar -vcf ~{filename}.preprocessed.vcf   --output-dir .  -phenotyper --matcher
    python /home/stereonote/script/genotype_phenotype_handle.py --phenotype_result ~{filename}.phenotype.json --outcall_file ~{outfile}
    java -Xmx1g -jar /home/stereonote/pharmcat-3.0.1-all.jar -vcf ~{filename}.preprocessed.vcf  -po ~{outfile} -reporterJson --output-dir .  -reporterCallsOnlyTsv 

  >>>
  runtime {
    #The fixed parameter "docker_url" is used in wdl to specify the image address,Please copy the real url from "Image" and paste it here ,such as"stereonote/stereonote:latest". If you switch the area,please change the image url
    docker_url: "stereonote_hpc/longrui_031af63faa1f4078bc6c5031da8512e5_private:latest"
    #The fixed parameter " req_cpu" is used in wdl to apply for a task running cpu
    req_cpu: 1
    #The fixed parameter " req_memory" is used in wdl to specify the running memory
    req_memory: "2Gi"
...
task preVcfPharmcatTask{
    ...
    command <<<
        ...
        /home/stereonote/miniconda3/bin/python  /home/stereonote/script/modify_alt.py ~{sample_id}.preprocessed.vcf results/pharmcat_ready/~{sample_id}.missing_pgx_var.vcf ~{sample_id}.preprocessed.new.vcf
    >>>

    output {
        File pharmcat_vcf_bgz = glob("results/pharmcat_ready/*.vcf.bgz")[0]
        File pharmcat_vcf = "~{sample_id}.preprocessed.new.vcf"
    }

    runtime {
        docker_url: "stereonote_hpc/longrui_214b0451e9434480b46748016cf9ed35_private:latest"
        req_cpu: 1
        #The fixed parameter " req_memory" is used in wdl to specify the running memory
        req_memory: "1Gi"
    }
...
task PostPharmCat {
    ...
    command <<<
        ...
        java -Xmx2g -jar /opt/conda/envs/bcftools/share/snpeff-5.2-1/snpEff.jar -v GRCh38.105  ~{pre_pharmcat_vcf} >annotated.vcf
         ...
         bcftools view -i 'QUAL>0' annotated.vcf.gz -Oz -W  -o qual_gt0.vcf.gz
        
        
        mkdir -p isec
         bcftools isec  -p isec -Oz -W ~{merged_unphased_vcf_gz}  qual_gt0.vcf.gz
         bcftools view  isec/0002.vcf.gz| grep -v '^##' > intersect.csv
        bcftools view  qual_gt0.vcf.gz| grep -v '^##' > annotated.csv
        ...
        python /home/stereonote/script/post_pharmcat_new.py \
            ...
            -short_guide_file_path "/home/stereonote/data/new_short_guideline_1018.xlsx" \
            -drug_name_path "/home/stereonote/data/drugs_1013.csv" \
            -drug_details "/home/stereonote/data/drug_details_1107_zw.xlsx" \
            -sample_id ~{sample_id} 
    ...
    runtime {
        docker_url: "stereonote_hpc/longrui_500a9bf378f4407fb1b7efded44a74e8_private:latest"
        req_cpu: 1
        req_memory: "3Gi"
  }
}
