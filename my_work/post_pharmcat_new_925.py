...
def get_report(report_json_file, diplotype_gene_set,gene_table):
    ...
    with open(report_json_file, 'r',encoding='utf-8') as file:
        data = json.load(file)
        cpic_all_guides:list = get_annotation(diplotype_gene_set, df1, data,DRUG_ANNOTATION['CPIC'],gene_table)
        dpwg_all_guides:list = get_annotation(diplotype_gene_set, df2, data,DRUG_ANNOTATION['DPWG'],gene_table)
        fda_all_guides:list = get_annotation(diplotype_gene_set, df3, data,DRUG_ANNOTATION['FDA'],gene_table)
        fda_pgx_all_guides:list = get_annotation(diplotype_gene_set, df4, data,DRUG_ANNOTATION['FDA-PGx'],gene_table)
        all_guides = (
            cpic_all_guides + 
            dpwg_all_guides + 
            fda_all_guides + 
...
def main():
    ...
    parser.add_argument("-report_json_file", type=str,help="Pharmcat report result file",default='my_work/data/test_new.report.json')
    parser.add_argument("-result_tsv_file", type=str,help="Pharmcat result tsv file",default='my_work/data/test_new.report.tsv')
    parser.add_argument("-outcall_file", type=str,help="pharcat outcall file",default='my_work/data/test_new_outcall.tsv')
    parser.add_argument("-intersect_file", type=str,help="pharmcat pipeline inputfile",default='my_work/data/intersect.csv')
    parser.add_argument("-prepharmcat_file", type=str,help="",default='my_work/data/annotated.csv')
